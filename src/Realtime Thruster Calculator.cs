    ...
    foreach(IMyTerminalBlock b in blocks){
        ...
        if(b.CustomData.Contains("Thruster LCD")){
            ...
            if(panel!=null){
                ...
                else wideMode = !b.DefinitionDisplayNameText.ToLower().Contains("panel");
...
public Planet getClosestPlanet(Vector3D pos){
    ...
    double closest = Math.Min(Math.Min(Math.Min(Math.Min(Math.Min(earthDist,titanDist),marsDist),alienDist),moonDist),europaDist);
